...
export default function ChatScreen({ relationship, session, onBack }: { relationship: RelationshipSummary; session: Session; onBack: () => void }) {
  ...
  async function send() {
    ...
    try {
      setBusy(true);
      if (editing) await editUnopenedMessage(editing.id, message);
      else await sendMessage(relationship.id, message);
      setMessage('');
      setEditing(null);
      await refresh();
    } catch (error) {
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          {messages.length === 0 ? <Text style={styles.empty}>No messages yet.</Text> : messages.map((item) => {
            ...
            return (
              <View key={item.id} style={[styles.bubble, mine ? styles.mine : styles.theirs]}>
                <Text style={styles.meta}>{mine ? 'You' : 'Other person'} · {new Date(item.created_at).toLocaleString()}{item.edited_at ? ' · edited' : ''}</Text>
                {hideIncomingBody ? <>
        ...
        <View style={styles.card}>
          ...
          <Button title={busy ? 'Saving…' : editing ? 'Save changes' : 'Send'} onPress={() => void send()} disabled={busy || !hasText || !result.canSend} />
          {editing ? <Button title="Cancel edit" onPress={() => { setEditing(null); setMessage(''); }} secondary /> : null}
        </View>

...
export async function shareConversationPdf(title: string, participants: ExportParticipant[], messages: ChatMessage[]) {
  if (!(await Sharing.isAvailableAsync())) throw new Error('File sharing is not available on this device.');
  const { uri } = await Print.printToFileAsync({ html: conversationExportHtml(title, participants, messages) });

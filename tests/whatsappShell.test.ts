...
test('Premium edge hard-blocks degrading language before an AI call', () => {
  for (const source of [
    ...
    assert.equal(
      premiumEdge.includes(source),
      true,
      `Missing Premium hard-block source: ${source}`,
    );
  }

  assert.match(premiumEdge, /degrading language are not allowed/);
  assert.ok(
    premiumEdge.indexOf('const hardBlock = hardBlockedFragment(message)') <
      premiumEdge.indexOf('consume_ai_analysis_for_user'),
  );
});

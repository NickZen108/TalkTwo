  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={t('common.back')} onPress={onBack} style={styles.backButton}><Text style={styles.back}>{t('common.back')}</Text></TouchableOpacity>
        <View style={styles.card}>
          ...
          <TextInput
            accessibilityLabel={t('feedback.feedback')}
            multiline
            value={message}
          ...
          <TouchableOpacity accessibilityRole="button" disabled={busy || !message.trim()} onPress={() => void send()} style={[styles.button, (busy || !message.trim()) && styles.disabled]}>
...
function makeStyles(colors: AppColors) {
  return StyleSheet.create({
    ...
    backButton: { marginTop: 16, minHeight: 44, alignSelf: 'flex-start', justifyContent: 'center', paddingHorizontal: 2 },
    back: { fontWeight: '800', color: colors.text, fontSize: 16 },
    ...
    chip: { minHeight: 44, justifyContent: 'center', borderWidth: 1, borderColor: colors.borderStrong, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: colors.surface },

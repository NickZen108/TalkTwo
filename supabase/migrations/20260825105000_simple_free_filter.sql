-- Replace the semantic Free-tier filter with a deliberately simple mechanical
-- policy. The Free tier no longer tries to infer blame, emotions, generalisations
-- or intent. It blocks only clear surface-form signals that are easy to explain
-- consistently: length, exclamation marks, emoji/emoticons, a conservative
-- Danish/English profanity/direct-insult list, excessive capitals and repetition.
--
-- Policy matching still uses NFKC + removal of default-ignorable formatting
-- controls so invisible Unicode cannot bypass these simple rules.
...
begin
  ...
  -- Emoji inspection must happen before variation selectors / formatting marks
  -- are removed from the canonical matching copy.
  if clean ~ '[😀-🙏🌀-🫿☀-➿]' or clean ~ '[🇦-🇿]'
  ...
  if lowered ~ '(^|[^[:alpha:]])(fuck|fucked|fucker|fuckers|fucking|motherfucker|motherfuckers|shit|bullshit|bitch|bitches|asshole|assholes|cunt|cunts|dickhead|dickheads|bastard|bastards|prick|pricks|wanker|wankers|twat|twats|idiot|idiots|moron|morons|fandme|fanden|satme|kraftedeme|krafteme|lort|lorte|pis|pisse|røvhul|røvhuller|kælling|kællinger|idioter|nar|narrer|svin|klaphat|klaphatte)([^[:alpha:]]|$)'
     or lowered ~ '(^|[^[:alpha:]])(fuck[[:space:]]+you|fuck[[:space:]]+dig|hold[[:space:]]+kæft|shut[[:space:]]+up)([^[:alpha:]]|$)' then
    return 'Profanity or direct insults are not allowed';
  end if;

  -- Mechanical repetition only. No attempt is made to infer why the user
  -- repeated something.
  if policy ~ '([?.;,])\1+'
  ...
  -- Allow short acronyms such as SMS / CPR, but stop obvious shouting.

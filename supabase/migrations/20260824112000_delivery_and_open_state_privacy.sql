...
-- whether an edit or withdrawal is still possible. Keep defensive stubs for
-- schema compatibility, but they are not callable from user clients.
...
revoke execute on function public.withdraw_message(uuid) from public,anon,authenticated;
revoke execute on function public.edit_unopened_message(uuid,text,text) from public,anon,authenticated;
...
grant execute on function public.withdraw_message(uuid) to service_role;
grant execute on function public.edit_unopened_message(uuid,text,text) to service_role;
